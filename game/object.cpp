...
void CObject::Process(float fElapsedTime)
{
	...
	if(!m_bAttached && m_iAttachVehicle && m_iAttachVehicle != 65535)
	{
		...
		if (pool->GetSlotState(m_iAttachVehicle))
		{
			CVehicle* vehicleObj = pool->GetAt(m_iAttachVehicle);
			if (vehicleObj && vehicleObj->IsAdded() && ((uintptr_t)vehicleObj->m_pEntity != g_libGTASA + 0x5C7358))
			{
				MATRIX4X4 vehMat;
				vehicleObj->GetMatrix(&vehMat);
				m_bAttached = true;
	...
	*/
	...
	if(m_iAttachVehicle != INVALID_VEHICLE_ID) {
		if(pNetGame) {
			CVehiclePool* pVehiclePool = pNetGame->GetVehiclePool();
			if(pVehiclePool) {
				CVehicle* pVehicle = pVehiclePool->GetAt(m_iAttachVehicle);
...
#ifdef USING_OBJECT_MATERIAL
#include "../game/objmaterial.h"
#endif
...
void CObject::SetMaterial(int iModel, int iMaterialIndex, char* txdname, char* texturename, uint32_t dwColor)
{
	if (iMaterialIndex < 16)
	{
		...
		#ifdef USING_OBJECT_MATERIAL
		m_MaterialTexture[iMaterialIndex] = (uintptr_t)LoadTexture(g_libGTASA, texturename);
		#endif
